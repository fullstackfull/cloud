...
| F-19 | High | CODE_GAP | `OPEN` — **upheld with reservations**, two blocking, in round four | The work itself is *"sound and unusually well pinned"*: one dispatch table (the parent's second is visible at `0c648ed:ServiceController.php:148-197`), `HowTheServiceEnded` confirmed a DTO with no dispatch so the duplication did not move, **16 of 16** independent mutations caught, and R4 and R5 checked **against the code rather than against my notes** — including one thing my own measurement missed, that `plans.product_id` is NOT NULL so the null arm also needs a soft-deleted product. **Block one is the cross-branch hazard I predicted, measured — and worse than I framed it, because it does not wait for F-18.** On this branch, a principal holding **only** `service.terminate` gets **403** on the hosting-account route and **202** on the service route, destroying the same live hosting account; `force` makes no difference on the service route, and under `force` its requirement is a **proper subset** of the other's. `hosting_account.manage` is required at no point on the service path. `ServiceController`'s `abort_unless` re-asserts a permission the route middleware already demanded, so it **can never fire**, and its docblock's rationale — *"a role that may do the first should not thereby be able to do the second"* — is true in `HostingController` and false here, where it is the same grant asked twice. **The branch never tests the newly-permitted case**, and `grep` over the whole src diff for any record of the change returns **0 hits**. **Block two: *"the refusal and the ending cannot drift apart again"* is falsified**, on the branch's own centrepiece row. `RetryProvisioningJob::execute` never consults `failure_class`; measured, the ending refuses the timed-out create and the retry **accepted it and dispatched a second create at the provider**. Sharing the method does not deliver the property, because the two callers ask different questions. Two refinements worth keeping: **channel 3 is load-bearing for VPS but not for hosting**, since a hosting create carries `providerReference` even on failure and is caught by channel 1; and **`C1` alone does not reproduce the original measurement** — the evidence channels block the leak independently of the dispatch, so it takes `C1+E3` to get the 202. |
