...
| F-14 | High | CODE_GAP | `OPEN` — in verification, **round eight**, rejected six times | The round I briefed on three date doors **refused my framing of one of them and widened it by a factor of fifty-nine**, and every claim it made reproduced exactly when I measured it myself. On the mixed separator it found the rule generalises further than I stated — the slash **does** mix in one direction (`01/02.2099` reads as 2 January, and the year is read only when its last two digits are ≥ 60, swept 1990–2100) — *"which the docblock I was about to write would have denied."* On the two-digit year it found the real fact is larger than my construction: with three short fields **the separator decides which of the three is the year**, not which of the first two is the month — `26-12-01` is 1 December 2026 and `26.12.01` is 26 December 2001, **a 25-year swing on punctuation alone**. Its fix is one rule rather than a list: a numeric date is refused unless one field is four digits and is therefore unmistakably the year, with the over-twelve reasoning kept *behind* that condition, because only with the year pinned are the first two fields a day and a month at all. **On the whitespace item it refused my framing outright**: NBSP is one of **fifty-nine** prefixes that leave a date readable — 48 of them plain letters — so a strip-list keyed on NBSP would have left 50 doors open; and a leading letter does not merely bypass the gate, **it moves the day**: I confirmed `V2099-01-01` parses as 1 **May** and `X2099-01-01` as 1 **October**. 26 mutations, 24 caught, both survivors shown unobservable by measurement rather than argued. 373 expiry spellings swept before and after: **35 verdicts change, every one accepted → refused, none the other way.** |
