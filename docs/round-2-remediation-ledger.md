...
| F-14 | High | CODE_GAP | `OPEN` — in rework, **rejected six times** | Round seven **rejected** `3665378`, and I verified its three central measurements myself on the installed PHP rather than on its word. The backreference's stated reason — *"a value punctuated two different ways is not a date PHP reads at all"* — is false: `date_parse('01-12.2026')` returns `error_count=0, y=2026 m=12 d=1`, and the node sells. The alternative the commit argued against (`[/.-]` as a second character class) closes it at **zero cost: 336/336 still green**, so the design choice was argued at length and pinned by nothing. Worse, *"a field over twelve can only be a day"* is false for a two-digit year, and **the dash this commit added is the only separator where it bites**: `30-01-12` reads as 2030-01-12 and sells, while `30.01.12` is safe by accident (2012, past) and `30/01/12` is safe by accident (`date_parse` errors). A `%d-%m-%y` panel — an ordinary strftime format — sells. A non-breaking space also reopens the *slash* form closed in round five, because PCRE's `\s` without `/u` is not U+00A0 while `date_parse` tolerates it. **None of this diminishes the round's best work**, which the verifier names as such: it probed 24 malformed pair spellings and established that no single raw pair can yield more than one leaf, so `held < sent` is a **true invariant** rather than a `%00` rule, and it confirmed by its own ~75-form sweep that the widened separator caused no fleet-wide over-refusal. |
...
| F-29 | Medium | SECURITY | `OPEN` — **rejected**, in rework | Round three ran **56 mutations** (the implementer ran 20), swept 60+ spellings and a 256-byte fuzz on both roads, and **could not move the byte rules**: all seven named families refused *with the right reason*, range arithmetic correct at first-inside/last-inside/first-outside for all twelve ranges, the corrected CPython prose measured true on the installed Python, every byte claim in the new docblocks measured true, and the write-road inventory confirmed with no fourth HTTP road missed. It rejected on the **enumeration around** those rules. **One leading separator defeats three rules at once**: `isNumericForm()` asks whether *every* label is numeric, so `explode('.', '.169.254.169.254')` yields an empty first element that fails `NUMERIC_LABEL`, the string is handed to the resolver as a name, and `HOST_CHARACTERS` admits a leading `.` or `-`. Measured over the real roads: `.169.254.169.254`, `-169.254.169.254`, `.0x7f000001` and `.metadata` all **201** — the implementer's own eight-hostname table with four more spellings. `NUMERIC_LABEL`'s docblock sets the standard it fails: *"a hostname's last label is a TLD and a TLD is never a number"* — and `.169.254.169.254`'s last label **is** a number. **This repository already asks that question correctly 200 lines away**: `DomainName::fromString()` refuses a numeric last label and `assertLabel()` refuses an empty label and a leading hyphen, so the positive shape rule chosen *"rather than another list of things to refuse"* is weaker than the label rule the same codebase ships. Also held: the AAAA gap is a **supported operator path to a credentialed socket** (an AAAA-only name resolves to nothing, is accepted, and `WhmConnection::forNode()` dials it with the WHM root token) — and the customer-facing `HttpSiteProbe` already queries `DNS_A|DNS_AAAA` and fails closed, so the root-credentialed operator surface is weaker at the same question than the page-fetching customer one; the corrected docblock is itself wrong (three roads, not two — `bmc_endpoints.address` reaches `BmcConnection` with the controller password, by way of a **customer** route); the environment dial on the PUT road the implementer *discovered* is unpinned while the one it was *told about* is pinned in both directions; and **22 test cases still put real DNS on the wire, two of them in files this round added** — including the very test written to pin that dial. |
...
| F-35 | Medium | OPERABILITY | `OPEN` — in rework | The finding is real, fixed, and pinned by a genuine **reachability** oracle at the command's output — the verifier built its own six-cell matrix through the operator's command on both trees rather than adopting the implementer's, confirmed 0/6 on the parent, and found **no over-strictness at all**: every failure direction it found is the check being too *generous*, never too strict. It **confirmed both ways the finding understates**: the false green also occurs when pools exist but all are switched off, and on the one half of the matrix where the check did run it gave a **wrong answer** for an all-inactive estate — so the parent held a reachability defect and a correctness defect in the same three lines. It **upheld the `scopeActive` change and told me not to back it out**, upheld leaving the estate scope alone, and upheld the `ManagedServer`/`DedicatedServer` paragraph. Held open on two. **The inventory states a false absolute** — *"this chain never drops anything"* of `ProviderChain`, which drops five rungs at `:106` — and the inventory's whole value is that it is true. And **the durable rule is not live on two of the five products it tabulates**: `wordpress` and `gpu_compute` are in `MAPPING_CHECKS` and in no estate shape, so **F-35's exact defect can be reintroduced for `gpu_compute` and the rule will not notice** (measured: survived 40/40). A third, non-blocking but owed: the repaired check still passes a management-scope, a v6-only and a zero-subnet pool, so the sentence justifying the fix is still true of three shapes after it — the implementer carried one of the allocator's four conditions across and wrote prose claiming its authority for all of them. |
