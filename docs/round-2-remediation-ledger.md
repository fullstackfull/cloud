...
| F-18 | High | AUTHORIZATION | `OPEN` — in verification | Hosting destruction control inverted. Round three found the case the fix exists for pinned only at unit level; the rework gave it four route-level rows and first-fail-proved them. Its prose audit is the most thorough of this programme: **nine false or materially misleading claims found and corrected across ~600 prose lines, two of them written in that same round**, including one that had stood in four places — *"`suspended_at` survives an unsuspension"*, which is false, since `UnsuspendHostingAccount` has always nulled it and the real provenance is termination not clearing it plus a capacity re-arm. It also partly disagreed with its verifier, naming three cases where a call the verifier had called dead is load-bearing. The drifting refusal date was kept with an argument — there is no honest anchor, because the missing column *is* the anchor — and pinned in both directions. |
...
| ID | Sev | Class | Status | Notes |
|---|---|---|---|---|
| F-11 | High | DATA_INTEGRITY + TEST_GAP | `OPEN` — in verification | Cloudflare collapses all records at one `(type, name)`; `records()` unpaginated. Round two found an eighth door with a security consequence — a priority- and data-blind reconciliation key hiding an unauthorised certificate authority — and named its cause: three comparisons that disagreed, with a docblock asserting two were the same. The rework did not add two fields to a string. It made `ReconcileZones` delegate to the **same two tiers** `findAmong()` already applies, so the module no longer has a fourth comparison that can disagree with `find()`, and produced a **27-site inventory** of every comparison in the module with the three genuine divergences stated in one line each. Then it found a defect **inside its own fix** and reported it unprompted: the new key's value tier was unreachable and untested, because both providers always return an identifier — *"an untested fallback in the one class that exists so this rule is stated once, which is precisely the defect class under review."* |
