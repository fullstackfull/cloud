...
| F-13 | Critical | CODE_GAP | `OPEN` — **upheld with reservations**, one blocking, in rework | The verification went past what it was asked and **closed F-13's headline on the real deployment path**, which the branch's own readiness test structurally cannot do because it hand-writes `readiness` and `state` onto the row. It read `ProviderReadiness::assess()` in full — it inspects `reached()`, `usable()` and `capabilities->isEmpty()` and **never a capability's state** — and then read `infrastructure/ansible/roles/proxmox/tasks/api_token.yml`, which does `pveum acl modify / -user … -role LynomiaProvisioning` with **`--privsep 0`**. So a cluster built by this repository's own automation really does hand the control plane a token holding all 20 privileges at the root of the resource tree. Both halves had to hold: without `--privsep 0` the deployed token would hold nothing and F-13 would be alive in a new form; with a readiness gate on capability completeness, `gpu_passthrough`'s permanent `Unknown` would reinstate it. All four mutants the round-three blocker named are **killed**. The 28-pair deletion sweep gives **0 survivors**, and still 0 with the oracle weakened to a subset check — which shows the counts test catches every deletion on its own and the oracle contributes exactly one failure per pair. Eleven empty-entry variants (the commit claimed seven), 0 survivors. It enumerated **30** in-code decisions against the commit's nine and found the commit's structural claim **confirmed rather than refuted**: every killed in-code decision was killed by a hand-written case, never by the generated machinery. It verified each dated narrative names the right pair count for the commit it dates itself to (29/27/28/28), and re-derived by an independent route that the removed assertion was authored by the defect's own commit. **The one blocker is a bound on F-13's own closure.** A token holding all 13 map privileges plus `Sys.Audit` but **not** `Datastore.Audit` is declared VPS-ready — `listStorages()` is its only user, reached only from `listNodes()`, reached only from `SyncClusterInventory`, which populates the storage rows placement reads. *"Declared ready, cannot build"* — F-13's own sentence — survives in that shape. The commit **names it precisely**, which is why this is a reservation and not a rejection, and its stated reason for leaving it is that inventing a capability would violate `EveryDeclaredCapabilityHasAConsumerTest`. The rework is told to test that premise before accepting it, since `SyncClusterInventory` looks like a real consumer; if the premise does not hold, a bound was accepted on a reason that was not true. No test pins it today and nothing tells the operator. Four more reservations are prose or overstatement, and the sixth corrects the commit's own record of a survivor **in both directions**: `O5`'s stated shape (dropping `create` from the `canWrite` list) is a provably **equivalent** mutant, because `create`'s privileges are a superset of `destroy`'s and `resize`'s; the real `O5` is a larger edit that makes a token which can create and destroy machines report *"holds no privilege that changes a virtual machine"*. |
| F-14 | Critical | DATA_INTEGRITY | `OPEN` — round ten plus two ruled-in defects delivered at `8cc7ec2`, **in its eleventh verification** | Nine rejections, every one of them for a vocabulary rule outflanked by a token the previous round had not thought of. Round ten was briefed on the structural frame instead — that `date_parse` **reports** a relative part while `CarbonImmutable::parse` **applies** it — and built an **outcome** rule rather than a word list: after the reader has run, refuse if `date_parse` warned at all, or if the day the reader produced differs from the day the gate reported. *"It cannot be outflanked by a token nobody thought of, because it never looks at tokens."* The second limb catches everything that moves the day; the first catches the one thing it structurally cannot — **a month named twice**, where `date_parse` agrees with the reader because it took the name as the month too, so nothing in the system knows except the warning. That is 12 month names and their abbreviations, a third of the previous round's word list, closed by a rule that never enumerates them. Measured through the shipped method itself on both sides, not a re-implementation — and the round re-took **every** published figure through the real methods after validating a faithful copy over 472,344 rows, *"because a re-implementation is exactly where a sweep stops measuring the thing it is about"*, which produced three corrections to its own first draft. Leading positions: 6,946 day-moving rows over 673 distinct tokens → **0**. Trailing: 11,580 over 1,077 → **0**. Over-refusal: of 5,554 rows previously accepted on the right day, the new rule refuses **two**, both of which name their month twice. The fleet's own 17 formats across every day from 2020 to 2100 — **502,945 values, 0 refused, 0 stored on the wrong day**. `Thu, 31 Dec 2099` licenses and `Fri, 31 Dec 2099` does not: one word apart, and that is the whole rule. **24 mutants, 23 killed**, the survivor reported rather than hidden and its gate kept anyway on a stated half-million-input measurement that is *"a measurement, not a proof"*. **Two defects it found, measured, and correctly did not fix on its own authority — I have ruled both in.** The numeric branch on the first line of `parseTimestamp()` sits outside the `try`, so a panel-controlled `expires=1e15` throws clean out of `SyncHostingNodeHealth` (bisected boundary: 99,999,999,999,999 returns, 100,000,000,000,000 throws). And a millisecond timestamp reads as **year 131968**, licensing a node effectively for ever — which is F-14's own harm class reached through the same method. My instruction on the second is the one that matters: **refuse, never reinterpret.** Nothing in the repository establishes that this panel ever sends milliseconds, so dividing by a thousand would be inventing a provider contract, and a wrong guess silently mis-licenses real nodes. **Both ruled-in defects are closed, and how they were closed is the difference between this round and the nine that were rejected.** The unguarded numeric return is now inside the `try` that already sat three lines below it, and its first-fail proof is the sharpest this programme has produced: the new test deliberately does **not** wrap the call, so against the pre-fix method it reports `errors=1` rather than `failed=1` — **the escaping exception is the evidence, rather than something asserted about it**. Eight spellings pinned across the bisected boundary, with `99999999999999` kept as a control so the guard catches what overflows and nothing else. On the millisecond ceiling I had given a constraint — refuse, never reinterpret — **without being able to say why refusing was safe**. The round answered that. It did not choose a horizon; it found that the method **already enforced 9999 on every path except the one that escaped**, from `isAmbiguousNumericDate()`'s four-digit-year rule and from `date_parse` refusing a five-digit year in every textual form, then measured it over **476,174 non-numeric values: 6,335 accepted, highest year produced exactly 9999, none above**. So the commit applies an existing invariant to the branch that escaped it rather than inventing a rule. And the headroom arithmetic is why refusing works without deciding anything: a second-precision expiry stays **ten digits until the year 2286**, and nothing is refused below **twelve**. Two orders of magnitude — which is how the rule separates seconds from milliseconds **without ever deciding which it is looking at**, and therefore without guessing at a provider contract. Both sides pinned to the second; the boundary pair differs by one second and straddles year 10000. Among the refused values is a licence *already expired* in seconds whose millisecond twin is 55,945 years ahead — the reinterpretation this refuses would have turned a dead licence into a live one. **29 mutants, 28 killed**, the survivor being the same documented `error_count` gate kept on a stated half-million-input measurement that is *"a measurement, not a proof"*. Round-ten's oracles re-run against the final method rather than assumed, all identical. Twelve bands, **4,150/4,150, 138,168 assertions, no `skipped` key in any band** (a band sum, not a full-suite figure). One self-correction recorded: its own *"still licenses"* control failed correctly, because the licence it had picked has genuinely lapsed as of today. A third defect found and **ruled record-not-fix**: `10000-01-01` is read by `date_parse` as year **2000** — it takes the first four digits and drops the fifth — so gate and reader agree and the new guard cannot see it; the value is past, so the node is refused. Safe by accident, like `Ymd`, and both are now labelled as accidents in the docblock rather than left to read as rules — and writing that sentence found a third instance the report had not: **`99999-01-01` reads as 2009**, the same dropped-digit behaviour one place further on, which is why the note is written as a class rather than around `10000`. |

### Wave 2 — concurrency, lifecycle, authorization

| ID | Sev | Class | Status | Notes |
|---|---|---|---|---|
| F-08 | High | CONCURRENCY | `OPEN` — round three delivered, **in verification** | Three rounds. Round two was held open on one blocker of mine and five reservations; round three closes all six and volunteers a seventh. **The blocker was that the boot-time check read the worker an operator hand-types and exempted the worker Horizon actually spawns** — I confirmed that gap myself before dispatching. The constant is now `WORKERS_TOLD_ON_THE_COMMAND_LINE` and holds all four commands that take a connection, a `--queue` and a `--timeout`; `horizon` itself stays out because the master supervisor takes none of the three. **I verified the rename on the branch**: `AppServiceProvider.php:84`, exactly those four entries, consulted at line 234 against `WORKER_COMMANDS` at 212. The half that decides whether this is safe to deploy is the converse — that the widened check does not refuse Horizon's *own* spawned workers, since a false positive there takes the platform's queues down on deploy. It is claimed proved by building the command strings Horizon really spawns, through the real `ProvisioningPlan::toSupervisorOptions()` → `SupervisorCommandString`/`WorkerCommandString`, for all five supervisors in all three environments — **30 processes, all accepted**, with the count asserted so an empty plan cannot pass vacuously, plus two real OS processes. **That claim is the verification's first job, and is not yet independently confirmed.** Round three also reports a trap it fell into and climbed out of, which is the kind of thing that usually goes unreported: parsing the spawned strings with `str_getcsv` kept the shell quotes, so the test asserted against a queue name that does not exist **and passed** — the refusal message was silently attributed to the wrong rule. The five reservations: the first-match-wins **direction** is now read off Horizon by running the real `deploy()` against a captured command queue rather than asserted a third time in the same direction; `HORIZONS_DEFAULT_TIMEOUT = 60` is applied at all three reads and taken off a real `SupervisorOptions` rather than off a comment; the development script must now actually **drain** every queue rather than merely be accepted; both bare-worker fallbacks are pinned; and `queuesInUse()` asks `QueuedClasses` instead of a regex. **Volunteered, not asked for — a fifth instance of the same absent-key mistake, one line below the third.** `SupervisorOptions::__construct` falls back `$queue ?: config('queue.connections.<conn>.queue')`, so a supervisor naming no `queue` watches its connection's own queue; `queuesOf()` read that as an empty list, so the supervisor contributed no ceiling and a worker pointed at the queue it really watches was measured against its own `--timeout` alone. 39 mutations, 38 caught, one declared intended survivor (a prose edit the new script parser is meant to be indifferent to). Four survived the first pass; three were real gaps and were closed. Reported figures awaiting confirmation: whole suite **3961/3961**, 144,408 assertions; `tests/Feature/Queue` 67/67; `tests/Feature/Simulation` 151/151; the three rule files **49/49, up from 30** at the parent. It also discarded two of its own earlier figures **with their causes**: a Simulation run that exited 144 with failures shaped like real regressions, caused by another agent's `pkill`, and a full-suite count taken one commit before head. |
...
| F-19 | High | CODE_GAP | `OPEN` — rework delivered at `81cb470`, **in verification** | Rejected twice before this round. Both blockers answered, three reservations closed, and **the volunteered root cause is the best part**: B2(b) was not a missing hosting arm, it was **two kind-dispatch tables** — `ServiceController::terminate` kept its own copy and only `EndOfService` had heard of shared hosting — so *"adding a third arm to the controller would have fixed the symptom and kept the defect."* There is one table now, and mutation `C1`, routing hosting to `endVps` in it, reproduces the original measurement and kills 5 tests. Unifying also made three previously dead null arms reachable from a real surface, which is what actually closes M24 rather than correcting its comment. The never-delivered list is stated **once** for all three kinds instead of three times, and the ending now asks the **builds** rather than a row's absence: four channels, of which the third is load-bearing — a create that timed out has `remote=NULL ref=NULL class='timeout'` and therefore **no identifier to point at**, so the retry-path helper alone would have missed exactly the case I set. That helper is now the same code, moved into the shared service and delegated, so the retry refusal and the ending read the same columns. **41 mutations, 40 caught, 1 equivalent**; an earlier identical pass found seven survivors, all closed by the middle commit. Full suite **3967/3967, 144,499 assertions**, no `skipped` key. It also refused R4 as unreachable rather than staging it, and corrected the false sentence instead — `PlanCapacity`'s *"`FulfilOrderOnSettlement` always writes one"* now says what is true, with the unreachability and its reason named. |
...
| F-26 | Medium | DATA_INTEGRITY | `OPEN` — rework delivered, **in verification** | The previous verification upheld the behaviour with nothing blocking, having reverted `ClaimZone` alone and proved the pre-fix 201 was **not cosmetic** (`findZone('db.panel.lynomia.test')` non-null — the zone really was created in the platform's own provider account). Four prose-and-derivation defects went to rework; six commits, `520111e`..`63f254f`, 17 mutations and **17 killed**. **The rework declined the obvious fix and was right to.** The derived reserved-zone list was empty for the exact configuration `.env.example` ships, because `localhost` is one label and `DomainName::fromString` refuses it. Rather than invent a name to reserve, it reasoned that **a single-label name cannot be claimed here either, so nothing is exposed by not reserving it** — and made the documents true and the silence loud instead, through what it calls the repository's own third answer, *report, don't refuse*: a new preflight check emitting one `dns.reserved_zones` finding, Warning when nothing is held, Warning naming the variable when a platform URL contributed nothing, and **Fail** on a malformed `DNS_RESERVED_ZONES` entry — the one genuinely blocking state, because `all()` parses the whole list before any comparison, so one bad entry refuses **every** customer's claim with an error about the customer's own domain. Report and guard read one source, so they cannot drift, and the report never quotes a reserved name or a configuration value. Verified end to end by running `infra:preflight` on the shipped configuration, not only in tests. **On the headline justification it found something better than its own assertion.** `zoneFor()` has no caller in `src/` — and the repository already knew: `NoDeadMethodsTest:148` lists it as reserved surface, and **its stated reason contradicts the use the old argument made of it**. The argument was replaced rather than patched, resting on two paths that do have callers, with the repository's own dead-method scanner offered as the evidence. **Two self-corrections committed rather than hidden**, both of a kind worth naming: one narrowed two sentences claiming the shipped config lets *"the first customer"* claim the control plane — false on a machine genuinely on localhost, *which is why deriving from it was not the fix*; the other removed a docblock claim of *"two years of this file's predecessors"* **from a file two days old** whose predecessor derived nothing from any URL. A docblock that invents its own history is a defect class, and the verifier is asked to look for others. |
| F-28 | Medium | SECURITY | `OPEN` — **upheld with reservations**, one blocking, in bounded rework | First independent verification, and it upheld everything but one docblock. F-28's sentence is **false of the code**; the second half — that the socket carries the token — is still true and is what makes it a security defect, and it verified that rather than assuming it, tracing the `PVEAPIToken` header onto the upgrade request and the vncticket into the path. **Both directions reproduce on the parent** through `AuthoriseConsoleConnection::execute()`, so the bidirectional claim is fully proved rather than half-proved. It **re-measured the TLS behaviour on its own loopback fixture with the flags enumerated separately** — the masking trap the implementer had disclosed — and confirmed `ssl://` defaults verification on and that both flags off completes a handshake against any certificate at all. It **tested the discriminator rather than inheriting it**, checking three endpoint tables, five consumers, Cloudflare, the identity tester and the one raw TLS socket in `src`: no counterexample, so the fix is not narrower than its justification. And it confirmed the factory line was genuinely blind in **both** directions on the parent before confirming it pinned in both at the tip. **The blocker is an impossibility claim that is false, and false for an instructive reason.** A test docblock said `verify_peer_name` *"cannot be [pinned] from this surface"* because *"the only trust anchor in play is the operating system's store, which a test cannot add to"*, and named a production change as the remedy. The verifier pinned it from exactly that surface, in both directions, with no production change — and the security half turns on the docblock's own reasoning: `putenv('SSL_CERT_FILE=…')` is honoured by OpenSSL's default verify paths, **which PHP falls back to precisely because the gateway's context names no `cafile`**. The stated obstacle is what enables the remedy it calls unavailable. The functional half needs no trust anchor at all, and is production-real: a self-signed certificate whose CN does not match the dialled address is the archetypal reason a row waives verification. It also reported a contradiction rather than reconciling it — the implementer's `180/180` against its own `182` on a named band — and found, outside the namespace, that `dedicated.bmc.verify_tls` / `REDFISH_VERIFY_TLS` is defined and **read by nothing**. |
| F-29 | Medium | SECURITY | `OPEN` — **upheld with reservations**, two blocking, in bounded rework | Four verification rounds; the fourth is the most thorough this programme has produced, and it closes the security substance. All seven address families are refused on **both** entry points with the right specific reason, against a **2,096-spelling fuzz that accepted none** — every `inet_aton` form of four sensitive addresses in decimal, octal and hex across 4-, 3-, 2- and 1-part, crossed with six decorators, plus 48 IPv6 embeddings, plus 636 more with UTS-46 separators, root labels, zone identifiers, control characters and fullwidth prefixes. It **attempted the construction that would have broken the last-label rule and proved it impossible**: the new rule refuses iff the last label is numeric, the old refused iff *all* labels were, and all-numeric implies last-numeric, so the new rule is a strict superset — confirmed by mutant M13 restoring the old loop and dying. The change traded nothing away. **No false refusals anywhere in the fleet**: every hostname, endpoint and address literal in `database/`, `src/`, `tests/` and `infrastructure/` was swept and only the intended negative fixtures refuse. **40 mutants, 36 killed**, the three survivors being exactly the three anchors the commit declares unpinnable, for exactly the stated reason — and it checked that declaration by turning each of the file's seven `\z` anchors into `$` in turn and counting which lines died: four died, three did not, which is the claim. It verified the BMC information disclosure closed **three deep** without taking the implementer's word, and found **a fourth road the implementer never mentioned** — `ReinstallDedicatedHandler:247` writes the address-bearing message into `dedicated_reinstalls.failure_message` — then established that column is withheld by `ReinstallRequestResource`, `CustomerOperationResource` and `ActivitySources` alike. And it proved **zero real DNS syscalls** through `SystemHostResolver` in the whole suite by putting the throw *at the syscall* rather than at `addressesFor()`, and logging as well as throwing so a swallowed `\Error` could not pass as a clean run: the suite is 4029/4029 and the log file was never created. **Held open on two.** The agreement test's docblock claims it judges *"every shape both of them judge, judged by both"* with *"one deliberate difference"*; it feeds **nine** shapes, and an exhaustive 66,048-label fuzz finds disagreements in two classes (underscore, asserted; **trailing-LF, undeclared**) — **the fourth verification put that count at 376 and it is wrong; the true figure is 111, and the correction is recorded below under my own name because I published the wrong one** with a 117-shape whole-name differential finding **five** divergence classes against the one asserted. The cause is one character: `DomainName::assertLabel` is anchored `$` where `HOST_LABEL` is anchored `\z`. Second, **an undeclared mutation survivor**: `M08` moves the 253-character host boundary by one and survives suite-wide, because the only over-length row in the entire test tree is 257 characters, so any mutant up to `> 256` passes — a fourth unpinned rule in a commit that declares exactly three. Three further false sentences and an F-40-shaped import set ride along non-blocking. |
| F-31 | Medium | SECURITY | `OPEN` — rework delivered, **in its sixth verification** | Five verification rounds found **no behaviour defect** — every rule survived a 19-mutation sweep, exhaustive address sweeps, an independent reproduction against genuine vendor code, and 885,452 probes returning 0 `true` and 0 throws — and the finding was held open on **three false sentences**. Seven commits, `4e8604f`..`8d5be5a`. **The rework confirms nothing round five settled was reopened**: `CANARIES` byte-identical, the colon-skip untouched. My ruling stands — F-31 closes with `TRUSTED_PROXIES=PRIVATE_SUBNETS` left open and only the reason moved. **The item with runtime teeth turned out worse than I briefed it, in three ways**, and the difference is not academic. I said an IPv6 entry on a no-IPv6 build made `$request->ip()` throw uncaught, a 500. Measured through a real request against a forced vendor build: the first throw is **not** `$request->ip()` but `HandleCors::hasMatchingPath()` → `Request::getHost()` → `isFromTrustedProxy()` — the middleware immediately after TrustProxies — so it fires on **every route** whether or not anything reads the client address; it is **not a rendered 500**, because `Handler::prepareResponse()` → `Response::prepare()` → `isSecure()` throws again from inside `renderException()`, outside the `catch` that called it, so the request dies as an **uncaught fatal with no body**; and **`/up` is in the band** — the health endpoint a balancer polls to decide a node is alive. Root cause of all three, and of the false sentence: `isFromTrustedProxy()` dispatches on the shape of the **peer**, not of the entries, and `REMOTE_ADDR` is a string nginx hands php-fpm, so PHP needs no IPv6 support to receive one. The fix probes the peer through the existing `covers()` seam and trusts nobody for that request when the build cannot evaluate it, making the throw **unreachable rather than caught** — it could not be caught in the middleware, because it is not raised there. **The reproduction was done correctly**, which is worth recording because an earlier F-31 agent is the reason the rule exists: it used `unshare-vendor-package.sh` to force `checkIp6()` in a **private copy**, confirmed the canonical tree untouched, and restored with a byte comparison afterwards. Seven of eight subclass seams are now `final`, the claim narrowed from *"cannot re-open"* to *"one method wide"*, pinned by a reflection test that asks the class rather than a hand-written list so a framework upgrade adding a protected method goes red. Figures: the F-31 file 34/34; `tests/Feature/Security` 257/257; full suite **3952/3952, 144,288 assertions, 0 failed, 0 errors** — and `skipped=0` argued **structurally** (three `markTestSkipped` sites; two convert to `fail()` under `CI=1`; the third gates on `pcntl_fork`/`posix_kill`, both present) rather than read off a counter. It **superseded its own earlier full-suite figure** of 3952/3943/9 errors, having established the 9 were its own concurrent run against the same database. |
