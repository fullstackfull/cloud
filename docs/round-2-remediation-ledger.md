...
| ID | Sev | Class | Status | Notes |
|---|---|---|---|---|
| F-08 | High | CONCURRENCY | `OPEN` — in rework | `retry_after` 90s below every supervisor timeout; seven listeners on `payments` re-execute concurrently with themselves. Fixed at two layers — three queue connections each with `retry_after` strictly above its supervisor's timeout, plus an attempt-keyed idempotency marker on the dunning counter. **Verifier refused the implementer's oracle and built its own with a money consequence**, holding the subscription row `FOR UPDATE` on a second connection rather than faking slowness: on the parent, one declined card gave `failed_payment_count = 2` across two real OS processes, the second starting exactly `retry_after` after the first. On the fix, 1 — with both workers still running the message, which is what proves the guard load-bearing independently of the config. It checked the relationship across all five supervisors and all five queues with no off-by-one, and confirmed a genuinely failed job is still retried. **UPHELD WITH RESERVATIONS.** The one that matters: `RecordInvoiceRefund` treats an unreadable `Refund` row as a first recording, so `amount_refunded_minor` grows on every execution — pre-existing, but the fix's own account claims that listener "reduces nothing twice" without qualification, and correcting `retry_after` leaves queue retries as the surviving duplicate source, which is exactly when a row lookup transiently fails. |
...
| F-18 | High | AUTHORIZATION | `OPEN` — in rework | Hosting destruction control inverted; `retentionHasElapsed()` true whenever `suspended_at` is null. Three verification rounds. Round two watched the **unattended scheduled sweep destroy a live, reactivated, paying customer's account** and confirmed that no seeded role holds `hosting_account.manage` without `service.terminate`, so the controller split is hardening and the predicate plus status guard are the fix. Round three returned **UPHELD WITH RESERVATIONS** — *"F-18 is genuinely closed at `cdadb51`"* — after deriving the whole state × date-shape × force matrix independently and establishing that the reorder moved **no cell between REFUSED and ALLOWED**, only four refusal *types*, and that no caller anywhere catches either exception by type, so nothing but the sentence a person reads could change. It also ran the control the implementer had not: the sweep still destroys a genuinely cancelled, expired account and is idempotent on replay. Reservations: the unelapsed stale row — the exact case the commit exists for — is pinned only at unit level; a third false claim (a matrix docblock describing three states over a six-state, 24-row table); and a customer-visible refusal date that drifts on every retry. |
...
| ID | Sev | Class | Status | Notes |
|---|---|---|---|---|
| F-11 | High | DATA_INTEGRITY + TEST_GAP | `OPEN` — in rework | Cloudflare collapses all records at one `(type, name)` onto `$existing[0]`; `records()` unpaginated at 100 against a 250 ceiling. Answered not with a seventh hand-written case but with a field-exhaustive equality table reading `DnsRecord`'s fields by reflection — which then found a TTL-only edit never reaching the provider, on unmodified code, with the simulator passing throughout. Round-two verifier reproduced all three limbs itself, judged every row of the table correct, and **disproved the over-strictness worry experimentally**: against a transport that clamps every TTL, three publishes gave `rows=1 POST=1 PUT=2`, so no duplicate is ever created, and `dns:reconcile` has no publish path at all so it cannot loop. It also corrected the implementer in its own favour. **Then it found an eighth door with a security consequence** — see the discovered table. |
