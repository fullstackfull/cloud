...
| F-28 | Medium | SECURITY | `OPEN` — rework delivered at `ad46b35`, **awaiting second verification** | First verification upheld everything but one docblock: F-28's sentence is false of the code, both directions reproduce on the parent, the TLS measurement was re-taken on its own fixture with the flags **enumerated separately** (the masking trap the implementer had disclosed), and the per-cluster/global discriminator was **tested rather than inherited** across three endpoint tables, five consumers, Cloudflare, the identity tester and the one raw socket in `src` — no counterexample. **The blocker was an impossibility claim, false for an instructive reason**: a docblock said `verify_peer_name` *"cannot be pinned from this surface"* because the OS trust store is one *"a test cannot add to"*. The verifier pinned it in both directions from exactly that surface with no production change — and the security half turns on the docblock's own reasoning, since PHP honours `SSL_CERT_FILE` through OpenSSL's default verify paths **precisely because the gateway's context names no `cafile`**. The rework deleted the four sentences, brought three rows into the shipped suite, and **all four mutations now die with the control surviving all four**. Its `src` diff contains **no executable lines**. It also **reconciled the 180/182 contradiction by arithmetic and measurement** — the commit added exactly 7 tests, 189 − 7 = 182, confirmed non-destructively — and concluded the implementer's 180 *"is not reproducible from any band the commit names"* and should be struck rather than reconciled. And it replaced a *"cannot"* with a genuine residual stated as **what was attempted and did not reach**: that the context names no `cafile` is itself unpinned, and pinning it needs a certificate in the OS store, which this test cannot place. |
