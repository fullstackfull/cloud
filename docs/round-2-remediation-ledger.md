...
| F-18 | High | AUTHORIZATION | `OPEN` — in verification | Hosting destruction control inverted. The security conclusion was upheld at round four; the finding stayed open for two defects that held under any standard. Both are now addressed, and the rework **found the query over-matched worse than reported** — eight rows where two are wanted, against twelve fixture shapes including two the verifier had not built — then **tested each clause and found two of six redundant**, catching its own draft that had claimed every clause was load-bearing. It **deviated one character from the prescription** with a reason (`<=` not `<`, because the sweep it claims to diagnose uses `<=`) and **corrected the verifier's own "narrows to A+B only"**. The query is now pinned by reading the SQL back out of its docblock by reflection and running it. The magnitude fix hit a real trap — the bound serialises to whole seconds while the clock carries microseconds — and was closed by freezing time rather than by a tolerance, on the argument that a tolerance would widen the hole back out. Fifteen claims corrected, three in its own new prose; one item I had named as four places was in nine. |
...
| ID | Sev | Class | Status | Notes |
|---|---|---|---|---|
| F-11 | High | DATA_INTEGRITY + TEST_GAP | `OPEN` — in verification, **fourth round** | Three rounds, each finding a door inside work the previous round declared complete: the eighth was an unauthorised CAA issuer invisible to the drift sweep; the ninth was the load-bearing sentence *"no fourth comparison that can disagree with `find()`"*, which was false, and the unification it invited left 465 tests green while turning a hijacked record from two drifts into zero. **The rework does not unify the comparisons — it writes the divergence down**, on the argument that `ReconcileZones::find()` must *not* read the identifier because noticing that the record under a known id has changed is reconciliation's whole job. It reproduced my claim first (replacing `find()`'s body with `findAmong()`: 181/181 green), then pinned it with a repoint fixture expecting two drifts, and restated the count in `docs/dns.md` as *"three comparisons and one deliberate narrowing, written down as four"*. **The 24-row inventory is now in the repository**, rebuilt by grepping the module rather than by reproducing the earlier prose — which is how it found its own earlier figure of 27 had double-counted sites across files. It also **corrected my own blocker**: I called the case narrowing "two lines in two files", and a blanket removal introduces a *new* false refusal, because `DnsRecord::of()` lowercases a record's name and not its content, so a CNAME target typed `Target.Example.test` is stored that way while the parser lowercases every host name it reads from a file. The narrowing is type-aware instead, through one statement on the type enum. A false customer-facing message was **deleted rather than reworded**, its branch having become unreachable. Both my corrections accepted, including that `DomainName::assertLabel()` refuses underscores so the DKIM example was unreachable. 490 tests / 21013 assertions / skipped=0. It does not declare F-11 closed. |
