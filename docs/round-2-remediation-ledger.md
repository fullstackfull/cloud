...
| F-18 | High | AUTHORIZATION | `OPEN` — in verification | Hosting destruction control inverted; `retentionHasElapsed()` true whenever `suspended_at` is null. Predicate repaired and a second permission layer added. First verifier raised six items, all addressed: the never-suspended case had been *borrowing* the retention-window refusal, putting English prose into `retention_releases_at`, a field typed as a Timestamp — now its own code, `hosting.termination_before_suspension`, in both locales. The rework also closed a path the verifier found in the unattended sweep: an account an operator had put back by hand was destroyed by the scheduled job, and that test fails on the parent. Two things flagged against the implementation by its own author: a narrowing of the `Terminated` replay, and the fact that re-inverting the predicate now has no HTTP catcher. |
| F-19 | High | CODE_GAP | `OPEN` — in verification | Nine of thirteen `OrderStatus` states unwritable; two exclusion clauses unreachable; `completed_at` never stamped. Eight states given production writers through one derived-state action plus three listeners; `Suspended` removed as fiction, with the argument that no business event produces it, it has no money consequence and no reader outside the enumeration. The money consequence was measured rather than asserted: a `stock_limit = 1` plan reported its unit claimed for ever after the machine was destroyed or refunded. Two existing oracles were found fail-open while repairing them — their fixtures have no node with room, so a delivered purchase and one nobody could build read identically. |

### Wave 3 — DNS, network, platform security

| ID | Sev | Class | Status | Notes |
|---|---|---|---|---|
| F-11 | High | DATA_INTEGRITY + TEST_GAP | `OPEN` — in rework | Cloudflare collapses all records at one `(type, name)` onto `$existing[0]`; `records()` unpaginated at 100 against a 250 ceiling; the simulator reproduces the defect. Verifier returned **UPHELD WITH RESERVATIONS**: all three limbs closed and independently reproduced, but it found a seventh breakage the suite does not catch — deleting the priority comparison from `DnsRecord::saysTheSameAs()` passes all 160 DNS tests while silently dropping a priority-only publish and blinding reconciliation to priority drift. |
