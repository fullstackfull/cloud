...
| Prepared products (CDN, object storage, GPU compute, email hosting) and readiness-only Kubernetes | `/readiness` | same routes; twelve products | `ProductReadinessEvaluator` software cap; `AssertProductMaySell` at the four sale actions in production | `product_readiness`; `not_implemented` blocker; declaration refused by name | `product_readiness.changed` | capped at `ready_for_real_validation`; 409 `readiness_refused`; 409 `product.not_sellable` on a new sale in production | `PreparedProductsCannotOutrunTheirSoftwareTest`, `PreparedProductsCannotBeSoldOnHopeTest`, `EveryDeclaredCapabilityHasAConsumerTest`, `EveryPreparedCategoryHasAContractTest`; `control-center-readiness.e2e.ts` | `RUNTIME_VERIFIED` — the cap, never the product |
| The twelve readiness answers | `/readiness` | `GET readiness/products` (`answers`) | `ReadinessQuestions` (derived from the verdict) | — | — | — | same | `RUNTIME_VERIFIED` |
| Record a GPU device | `/machines` | `GET|POST infrastructure/servers/{id}/gpus` · `infrastructure.manage` | `RegisterGpuDevice` | `gpu_devices`; counted as capacity only on a machine the platform may configure, in a passthrough mode | `infrastructure.gpu.registered` | 409 `gpu_exists`; recording never raises a classification | `AGpuIsRecordedAndCountedOnlyWhereItMayBeUsedTest`; `control-center-machines.e2e.ts` | `RUNTIME_VERIFIED` — the record, never the passthrough |
| Provider seats for prepared products | — | `GET providers/catalogue` | `CdnProvider`, `ObjectStorageProvider`, `EmailHostingProvider` (interfaces, no implementation); `smtp` catalogued over `LaravelMailTransport`, untestable | — | — | a class implementing a seat fails the gate while the addendum records no adapter | `EveryPreparedCategoryHasAContractTest`, `TheCatalogueOnlyClaimsWhatExistsTest`, `TheSmtpDriverIsCataloguedButNeverProvenTest` | `TESTED` — seats only |
...
| `lynomia_domain_redemptions_total{state}`, `lynomia_dns_zone_imports_total{outcome}`, `lynomia_backup_file_restores_total{state}`, `lynomia_country_currency_changes_total{state}`, `lynomia_wordpress_site_operations_total{kind,state}` | `ProductCollector`, `DnsCollector` — bounded label sets, no identifiers | — | `MetricsCarryNoIdentifiersTest`, `MetricsQueryBudgetTest` (42) |
| `DomainRedemptionNeedsReview` | `business.yml` | `docs/runbooks/domain-redemption-indeterminate.md` | `EveryAlertNamesARunbookThatExistsTest` |
| `FileRestoreIndeterminate` | same | `docs/runbooks/file-restore-indeterminate.md` | same |
| `CurrencyChangeNeedsReview` | same | `docs/runbooks/currency-change-needs-review.md` | same |
| `WordPressPushIndeterminate` | same | `docs/runbooks/wordpress-push-indeterminate.md` | same |
