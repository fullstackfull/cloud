...
return [
    ...
    'api.v1.orders.store' => [
        ...
        A shared-hosting item carries `domain`, the name the account is for:
        required on a hosting item (`checkout.domain_required`), refused on any
        other (`checkout.domain_not_applicable`), and held to the platform's
        host-name rule (`checkout.domain_unusable` — no scheme, path, port or
        non-ASCII; an internationalised name is sent as its `xn--` form). It is
        folded to lower case without surrounding whitespace or dots, and the
        folded name is part of the basket's fingerprint: the same key with a
        different domain is a different purchase.
        TEXT,
    ...
    'api.admin.hosting_accounts.password_reset' => [
        'tag' => 'Operator',
        'summary' => 'Set a new panel password on a hosting account',
        'description' => 'The operator\'s way into an account whose create answer was lost: the panel may hold it under a password the platform generated and, by design, kept nowhere. The new password is generated by the platform - none is accepted in the body - set at the panel, and returned in this response once; it is stored nowhere, including the audit entry, which records who, which account and why. Refused (409, `hosting.password_reset_refused`) for an account the panel no longer holds; a pending account is allowed, because that is the lost-answer state. Its own permission, not hosting_account.manage, and three a minute per operator.',
        'permission' => 'hosting_account.reset_password',
        'body' => ['reason'],
        'response' => $one('AdminHostingPasswordReset'),
    ],
    ...
    'api.admin.services.terminate' => [
        'tag' => 'Operator',
        'summary' => 'End a service and destroy what was built for it',
        'description' => 'The one act on this surface that destroys data, through the same path the retention sweep uses. The retention window on a suspended service is enforced by each kind\'s action, not by the caller: most suspensions are billing disputes that end with the customer paying. `force` skips each kind\'s suspension-and-window guard — so a service that is not suspended at all may be ended — for an abuse case or a right-to-erasure request, and is recorded as forced; it adds no permission and removes none. The route needs `service.terminate`, and a shared-hosting service needs `hosting_account.manage` as well, asked whether or not `force` is sent — so this route never asks less than the hosting-account route does for the same account. Answers 202. For a VPS the machine is destroyed by a worker, and the service reaches `terminated` when that worker succeeds, releasing the address into quarantine and the capacity back to the node. For a dedicated server nothing is queued: the machine leaves the customer and is held in `maintenance` until an operator states its disks have been erased, because no call this platform can make proves that they were. For shared hosting the account is removed at the panel (`hosting_account_id`) and the service ends with it. A service with nothing built for it — no machine, server or account, and a build history that could not have left one — ends with nothing destroyed; one whose build may have left something at a provider (a timed-out build, a task the provider accepted, a build still running) is refused 409 `provisioning.build_may_exist`, forced or not.',
    ...
    'api.admin.catalogue.products.index' => [
        ...
        'response' => ['envelope' => 'list', 'schema' => 'OperatorProduct'],
    ],
    'api.admin.catalogue.products.record' => [
        ...
        'response' => $one('OperatorProduct', 201),
    ],
    ...
    'api.admin.catalogue.plans.index' => [
        ...
        'permission' => 'catalog.manage',
        'response' => ['envelope' => 'list', 'schema' => 'OperatorPlan'],
    ],
    ...
    'api.admin.catalogue.hosting_packages.index' => [
        ...
        'permission' => 'catalog.manage',
        'response' => ['envelope' => 'list', 'schema' => 'OperatorHostingPackage'],
    ],
    'api.admin.catalogue.hosting_packages.map' => [
        ...
        'response' => $one('OperatorHostingPackage', 201),
    ],
    'api.admin.catalogue.hosting_packages.withdraw' => [
        ...
        'permission' => 'catalog.manage',
        'response' => $one('OperatorHostingPackage'),
    ],
