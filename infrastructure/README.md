...
| Script | Refuses |
| --- | --- |
| `validate-inventory.py` | A host with no `safety_class`, an `allow_reimage` its class does not permit, or a variable that reads like a credential — judged on the variables the script resolves for it from the environment directory's YAML sources and the `group_vars/` and `host_vars/` in that directory, by rules taken from ansible-core 2.18 — and a shape it has been found to read differently from Ansible: a source that is not YAML or holds an empty document, a variables file it cannot read (Vault-encrypted, say), a group key Ansible skips, a host range or port, a group loop, or no host at all. Those refusals are what attack has found, not a boundary; the script's docstring lists them, what it does not read, and how much of today's tree falls in each. What a play adds at run time, such as the `group_vars/` beside a playbook, is not part of the inventory and is not read — and `ansible-inventory` run from `infrastructure/ansible/` prints those too, so its output there is not this check's subject |
...
| `validate-runbook-alerts.py` | A runbook citing an alert no rule file defines, in a code span of the form the gate reads, or declaring a word not an alert when no code or configuration file names it (a `.md`, `.txt` or `.rst` file never counts), a stale count in the runbooks README, a page no alert names that the README does not list with a reason, or an alert with no page there that the README's list of them leaves out or keeps after it gains one |
| `test_validate_runbook_alerts.py` | The above gate quietly stopping to refuse the forms its cases pin for each of those, or its citation threshold drifting off eight |
