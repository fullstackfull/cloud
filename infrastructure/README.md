...
| Script | Refuses |
| --- | --- |
| `validate-inventory.py` | A host with no `safety_class`, an `allow_reimage` its class does not permit, or a variable that reads like a credential — judged on the variables the environment's inventory gives it, which is what `ansible-inventory -i <environment> --host <host>` prints: every YAML source in the directory, then the `group_vars/` and `host_vars/` in it — and an environment it cannot read that way: a source that is not YAML, a variables file it cannot read (Vault-encrypted, say), a group key Ansible skips, a host range or port, a group loop, or no host at all. What a playbook adds at run time, such as the `group_vars/` beside it, is not part of the inventory and is not read |
