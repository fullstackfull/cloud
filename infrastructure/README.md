...
| `validate-runbook-alerts.py` | A runbook citing an alert no rule file defines, or declaring a word not an alert when no code or configuration file names it (a document never counts), a stale count in the runbooks README, a page no alert names that the README does not list with a reason, or an alert with no page there that the README's list of them leaves out or keeps after it gains one |
