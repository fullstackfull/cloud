...
FRAMEWORK = {
    ...
    "optimize", "optimize:clear", "about", "down", "up",
}
...
def main(argv: list[str]) -> int:
    ...
    # Runbooks, and the playbooks that run artisan on a real host.
    targets = sorted(infra_root.rglob("*.md")) + sorted(
        (infra_root / "ansible").rglob("*.yml")
    )
        ...
        for line_no, line in enumerate(path.read_text().splitlines(), start=1):
            for command in INVOCATION.findall(line):
                ...
                if command not in known:
                    problems.append(
                        f"{path.relative_to(infra_root)}:{line_no}: "
