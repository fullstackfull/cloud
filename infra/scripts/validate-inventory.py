...
REQUIRED = ("ansible_host", "safety_class", "allow_reimage",
            "credentials_available", "purpose", "owner")
...
def hosts_in(node: dict, path: str = "") -> list[tuple[str, dict, str]]:
    ...
    for group, body in (node or {}).items():
        ...
        for name, hostvars in (body.get("hosts") or {}).items():
            found.append((name, hostvars or {}, here))
        found.extend(hosts_in(body.get("children") or {}, here))
    return found
...
def check_host(name: str, hostvars: dict, group: str, where: Path) -> list[str]:
    ...
    if reimage is not None and not isinstance(reimage, bool):
...
def check_file(path: Path) -> list[str]:
    ...
    hosts.extend((n, v, "all") for n, v in (root.get("hosts") or {}).items())
...
def main(argv: list[str]) -> int:
    ...
    for path in files:
        ...
        hosts = len(hosts_in((yaml.safe_load(path.read_text()) or {}).get("all", {}).get("children") or {}))
